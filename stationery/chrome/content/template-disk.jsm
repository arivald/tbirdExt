...
function fixImagesPathsInTemplate(htmlDocument, newPath, nodes, gMsgCompose) {
  ...
  function fixPath(node, attrib, asData) { //internal helper
    ...
    if(!filename.match(/^(http|https|chrome|file|data):/)) {
      ...
      try {
        ...
        node.setAttribute(attrib, filename);
          ...
          filename = node.getAttribute(attrib);
      }
    }
    ...
    if (asData && filename.match(/^(http|https|chrome|file):/))
      try {
        ...
        node.setAttribute(attrib, 'data:' + content.contentType + ';base64,' + btoa(content.content) );
      } catch (e) {
          Stationery.handleException(e);
      }
  }
  ...
  for (let i = 0 ; i < bodyNodes.length; i++) fixPath(bodyNodes[i], 'background', needBackgroundBugFix);

  let imgNodes = htmlDocument.getElementsByTagName('IMG');
  for (let i = 0 ; i < imgNodes.length; i++) fixPath(imgNodes[i], 'src', false);
}