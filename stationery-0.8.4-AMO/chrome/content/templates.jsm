...
let standardTemplateElements = ['type', 'name', 'url'];
...
Stationery.templates = {
  ...
  addNewTemplate: function(window, identityKey, handlerType) {
    let handler = handlers[handlerType];
    ...
    //effectivelly it will move existing template, instead adding new
    if (dup) template = dup; 
    //otherwise give it new ID
    else template.uid = generateUid();

    removeTemplateFromIdentity(identityKey, template); 
    let list = getListForIdentity(identityKey)
  ...
  setCurrent: function (identityKeyOrWindow, templateOrUid) {
    ...
    let list = getListForIdentity(identityKeyOrWindow);
    list.unshift(template);
    fixSingleIdentyList(list);
  ...
  getIdentitiesIterator: function(all /*bool*/) {
    ...
    for each (let account in fixIterator(MailServices.accounts.accounts, Components.interfaces.nsIMsgAccount))
      ...
      for each (let identity in fixIterator(account.identities, Components.interfaces.nsIMsgIdentity)) {
        let idKey = identity.key;
    ...
    Result.sort(function(x,y){ 
      let a = x.identity.identityName.toUpperCase(); 
      let b = y.identity.identityName.toUpperCase(); 
  ...
  getHandlerMenuitemIterator: function(doc) { 
    ...
    for (let i = 0; i < handlerTypes.length; ++i) {
      let menuitem = handlers[handlerTypes[i]].generateMenuitem(doc, 'menu');
  ...
  getHandlerOptionsAddMenuitemIterator: function(doc) { 
    let result = [];
    for (let i = 0; i < handlerTypes.length; ++i) {
      let menuitem = handlers[handlerTypes[i]].generateMenuitem(doc, 'options.add');
      if (menuitem) result.push(Stationery.setupElement(menuitem, {
        attr: [ {name: 'stationery-handler-type', value: handlerTypes[i] } ],
      }) );
    }
...
function findDuplicate(identityKeyOrWindow, template) { 
  let handler = handlers[template.type];
  let list = getListForIdentity(identityKeyOrWindow);
  for(let i = list.length - 1; i >= 0; --i)
