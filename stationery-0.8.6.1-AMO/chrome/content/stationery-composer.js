...
Stationery_.orginalMsgComposeCloseWindow = MsgComposeCloseWindow;
MsgComposeCloseWindow = function(recycleIt) {
  ...
  Stationery_.orginalMsgComposeCloseWindow(recycleIt);
}
...
Stationery_.ApplyTemplate = function() {
  try {
    ...
    if (gMsgCompose.type == mct.Reply || gMsgCompose.type == mct.ReplyToSender || gMsgCompose.type == mct.ReplyAll || gMsgCompose.type == mct.ReplyToList || gMsgCompose.type == mct.ReplyToGroup || gMsgCompose.type == mct.ReplyToSenderAndGroup)
...
Stationery_.gSourceEditorCommandTable = false;
Stationery_.getSourceEditorCommandTable = function() {
  if (!Stationery_.gSourceEditorCommandTable) {
    let controller = Components.classes['@mozilla.org/embedcomp/base-command-controller;1'].createInstance();
    let editorController = controller.QueryInterface(Components.interfaces.nsIControllerContext);
    editorController.init(null);
    editorController.setCommandContext(window);
    window.controllers.insertControllerAt(0, controller);
    
    Stationery_.gSourceEditorCommandTable = controller.QueryInterface(Components.interfaces.nsIInterfaceRequestor).getInterface(Components.interfaces.nsIControllerCommandTable)
  }
  return Stationery_.gSourceEditorCommandTable;
}
...
Stationery_.onIdentityChanged = function(event) {
  ...
  try {
    ...
    */
    //Stationery_.identityChangedNewSignature = 
