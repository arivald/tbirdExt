...
let valuesBranch = Services.prefs.getBranch('extensions.stationery.');
let defaultsBranch = Services.prefs.getDefaultBranch('extensions.stationery.');
  ...
  try {
    ...
    switch (preference.type) {
      ...
      case PT_JSON: 
        ...
        break;
      case PT_UNICODE: 
        let s = Stationery.XPCOM('nsISupportsString');
        s.data = value;
...
if ( valuesBranch.getPrefType('Templates') == valuesBranch.PREF_INVALID ) {
  Stationery.modules['prefs_old'] = {
    beforeInitWindow: function() {
      ...
      let oldBranch = Services.prefs.getBranch('stationery.');
      ...
      let makeAbbrevTamplateName = function (templateUrl) {
      ...
      //clear old prefs
        //todo: iterate oldBranch
      /*  
        [...].forEach(function(i) { try { 
          if (oldBranch.getPrefType(i) != valuesBranch.PREF_INVALID) 
            oldBranch.clearUserPref(i);
        } catch(e) {} });
      */
...
Stationery.definePreference('AddresingWidgetLines', { type: 'int', default: 2 });
